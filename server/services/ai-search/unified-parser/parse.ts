...
export async function parseQueryFast(query: string): Promise<QueryParseResult> {
  ...
  try {
    ...
    const needsClarification = classification.specificity === 'low' || confidence < 0.5; const queryType = classification.specificity === 'high' ? 'specific' : classification.specificity === 'medium' ? 'broad' : 'ambiguous'; const expandedFromOriginal = filters.jobTitles.length > 1 || filters.seniorities.length > 0;
