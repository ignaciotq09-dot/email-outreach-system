...
const tabs = [
  ...
  { id: 'workflows', label: 'Workflows', icon: Workflow, group: 'automation' },
  { id: 'sent', label: 'Sent', icon: Mail, group: 'monitoring' },
...
export function FigmaTabBar({ activeTab, onTabChange }: TabBarProps) {
  return (
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`
                relative flex items-center gap-2 px-4 h-full transition-all group cursor-pointer
                ${isActive 
            ...
            >
              ...
              {isActive && (
                <>
                  <div className="absolute bottom-0 left-2 right-2 h-1 bg-gradient-to-r from-purple-500 via-violet-500 to-indigo-500 dark:from-purple-500 dark:via-pink-500 dark:to-cyan-500 rounded-full shadow-lg dark:shadow-purple-500/80" />
