...
import { Mail, Eye, Reply, MessageSquare, Users, CheckCircle2 } from "lucide-react";
import type { OverviewMetrics, SmsOverviewMetrics, ChannelFilter } from "./types";
...
export function OverviewCards({ channelFilter, overview, smsOverview, isLoading, formatPercent }: OverviewCardsProps) {
  if (isLoading) {
  ...
  return (
    <>
      <Card data-testid="card-metric-combined-sent"><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 gap-2"><CardTitle className="text-sm font-medium">Total Messages</CardTitle><div className="flex gap-1"><Mail className="h-4 w-4 text-muted-foreground" /><MessageSquare className="h-4 w-4 text-muted-foreground" /></div></CardHeader><CardContent><div className="text-2xl font-bold" data-testid="text-combined-total">{(overview?.totalSent || 0) + (smsOverview?.totalSent || 0)}</div><p className="text-xs text-muted-foreground">{overview?.totalSent || 0} emails, {smsOverview?.totalSent || 0} SMS</p></CardContent></Card>
